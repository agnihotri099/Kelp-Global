        ...
        const query = `
            SELECT 
              CASE 
                ...
                WHEN age > 60 THEN '> 60'
              END as age_group,
              COUNT(*) * 100.0 / (SELECT COUNT(*) FROM users)::float as percentage
            FROM users
            GROUP BY age_group
            ORDER BY age_group;
        `;
...
const uploadCSVData = async (req, res) => {
    try {
        const csvData = await parseCSV(process.env.CSV_FILE_PATH);
        ...
        await Promise.all(validData.map(async (user) => {
            ...
            await pool.query(
                'INSERT INTO users (first_name, last_name, age, additional_info) VALUES ($1, $2, $3, $4)',
